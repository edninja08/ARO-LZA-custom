...
module "platform" {
  ...
  spoke_vnet_id              = module.vnet.spoke_vnet_id
  private_endpoint_subnet_id = module.vnet.private_endpoint_subnet_id
  acr_id                     = module.supporting.acr_id
  aro_sp_object_id           = module.serviceprincipal.sp_object_id
  spoke_keyvault_id          = module.supporting.spoke_keyvault_id
  aro_cluster_name           = var.aro_base_name
  aro_api_server_url         = module.aro.console_url
  aro_ingress_ip             = module.aro.ingress_ip
  la_id                      = azurerm_log_analytics_workspace.la.id
  random                     = random_string.random.result

  # Azure SQL Database configuration
  sql_admin_password       = random_password.sql_pw.result
  sql_aad_admin_login      = var.sql_aad_admin_login
  sql_aad_admin_object_id  = var.sql_aad_admin_object_id

  # Helm configuration — set these for automated chart deployment
